 ...
 * - Handles both Local and Cloudinary storage dynamically
 */
...
// GET /api/documents/by-category — grouped object
...
// POST /api/documents — upload PDF (DYNAMIC STORAGE)
router.post('/', protect, adminOnly, handleUpload(uploadPDF.single('pdf')), async (req, res) => {
  try {
    ...
    // Agar mode cloudinary hai, toh req.file.path mein Cloudinary ka URL hota hai
    const isCloud = process.env.STORAGE_MODE === 'cloudinary';
    const fileUrl = isCloud ? req.file.path : buildLocalUrl(category, req.file.filename, 'documents');
    const filePath = isCloud ? req.file.path : `/uploads/documents/${category}/${req.file.filename}`;

    const doc = await Document.create({
      title,
      fileName:    req.file.originalname,
      filePath,    // Ye database mein path save karega
      fileUrl,     // Ye frontend ke liye direct link save karega
      category,
...
router.delete('/:id', protect, adminOnly, async (req, res) => {
  try {
    ...
    // Delete physical file ONLY if it's local
